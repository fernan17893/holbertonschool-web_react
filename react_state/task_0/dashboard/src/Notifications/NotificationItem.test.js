...
describe("Testing <NotificationItem /> Component", () => {
	...
	it("Verify that when clicking on the menu item div, the handleDisplayDrawer is called", () => {
		let props = {
			displayDrawer: false,
			handleDisplayDrawer: () => {}
		};
		...
		let spy = jest.spyOn(instance, 'handleDisplayDrawer');
		wrapper.find('.menuItem').simulate('click');
		expect(spy).toHaveBeenCalled();
	}
	);

	it("Verify that when clicking on the button, the handleHideDrawer is called", () => {	
		let props = {
			displayDrawer: true,
			handleHideDrawer: () => {}
		};
		wrapper = shallow(<Notifications {...props} />);
		let instance = wrapper.instance();
		let spy = jest.spyOn(instance, 'handleHideDrawer');
